...
def homomorphic(cipher1, cipher2, p):
	...
	list_ci2 = list(map(int, cipher2.split()))

	# Ensure both lists have the same length and even number of elements
	if len(list_ci1) != len(list_ci2) or len(list_ci1) % 2 != 0:
		raise ValueError("Invalid ciphertext pairs.")

	cipher = []

	# Iterate through the lists in steps of 2 to handle pairs (C1, C2)
	for i in range(0, len(list_ci1), 2):
		c1_product = (list_ci1[i] * list_ci2[i]) % p
		c2_product = (list_ci1[i + 1] * list_ci2[i + 1]) % p
		cipher.append(f"{c1_product} {c2_product}")
